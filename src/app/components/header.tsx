    ...
    <>
      ...
      >
        <p className="text-xs sm:text-sm text-neutral-400 tracking-wide">
          <span className="text-[var(--accent)] font-semibold">American Book Founders</span>{" "}
          is owned by{" "}
          ...
          <div className="flex h-16 lg:h-20 items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="font-semibold">info@neonovallc.com</span>
              </a>
            </div>
          </div>
        </div>
      ...
      {/* Mobile Header for below lg screens */}
      <header
        style={{ ["--accent" as any]: ACCENT }}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 lg:h-20 items-center justify-between gap-4">
            ...
            <button
              ...
              onClick={() => setOpen((v) => !v)}
              className="lg:hidden inline-flex items-center justify-center rounded-lg border border-neutral-800 bg-neutral-900/80 p-3 text-white transition hover:border-[var(--accent)]/50 hover:text-[var(--accent)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]/60"
            >
              {open ? (
                <FiX size={22} className="text-white" /> // Cross icon in white
              ) : (
                <FiMenu size={22} className="text-white" /> // Hamburger icon in white
              )}
            </button>
          </div>
        </div>
      ...
      >
        <div className="fixed inset-0 z-40">
          ...
          <div className="absolute right-0 top-0 h-full w-full border-l border-neutral-800 bg-black p-6 z-50">
            {/* Mobile Navigation */}
                ...
                return (
                  <Link
                    ...
                    className={`block rounded-lg px-3 py-3 text-[1.05rem] font-semibold tracking-wide transition-colors
                      ${active ? 'text-[var(--accent)] bg-neutral-800/50' : 'text-neutral-200 hover:text-[#C6A15B] hover:bg-neutral-800/40'}`}
                  >
                    {item.label}
                    <span
          ...
          ${
            active
              ? "text-[var(--accent)] bg-neutral-800/50"
              : "text-neutral-200 hover:text-[#C6A15B] hover:bg-neutral-800/40"
          }`}
                  >
                    {item.label}
                    <span
      ...
      <div className="h-[2px] w-full bg-gradient-to-r from-[var(--accent)] via-[var(--accent)] to-[var(--accent)]" />
    </>
  );
}
